...
static void ddb_input_start(struct ddb_input *input)
{
...
#ifdef DVBNET_TEST
	{
		struct sockaddr_in cadr = { 
			.sin_family=AF_INET, 
			.sin_addr.s_addr=htonl(0xc0a80248),
			.sin_port=htons(31336)
		};
		int sndbuf=256*1024;
		if (sock_create_kern(AF_INET, SOCK_DGRAM, IPPROTO_UDP, &input->sock)<0)
			printk("no sock\n");
		if (kernel_setsockopt(input->sock, SOL_SOCKET, SO_SNDBUF,
				      (char *) &sndbuf, 4)<0)
			printk("SNDBUF failed\n");
		if (kernel_connect(input->sock, (struct sockaddr *) &cadr, sizeof(cadr), 0))
...
static u32 ddb_input_avail(struct ddb_input *input)
{
	...
	if (ctrl & 4) {
		printk("IA %d %d %08x\n", idx, off, ctrl);
		ddbwritel(stat, DMA_BUFFER_ACK(input->nr));
		return 0;
	}
	if (input->cbuf != idx)
		return 188;
...
int rtp_send(struct socket *sock, unsigned char *head, unsigned char *buf)
{
        struct msghdr msg = {.msg_flags = 0};
        struct kvec iv[2] = {{head, 12}, {buf, 7*188}};

	//return kernel_sendmsg(sock, &msg, iv, 2, 1328);
	return kernel_sendmsg(sock, &msg, iv+1, 1, 1316); //just TS
}

void ts_send(struct ddb_input *input)
{
	int i, res;
	u8 head[12]={0x80, 0x21,};

	for (i=0; i<96; i++) {
		head[2]=(input->sn>>8)&0xff;
		head[3]=input->sn&0xff;
		input->sn++;
		res=rtp_send(input->sock, head, input->vbuf[input->cbuf]+i*7*188);
		if (res<0) 
			printk("RTP %d\n", res);
	}
}
...
static const struct pci_device_id ddb_id_tbl[] __devinitdata = {
	...
	DDB_ID(DDVID, 0x0003, DDVID, 0x0010, ddb_v6),
	DDB_ID(DDVID, 0x0003, DDVID, 0x0020, ddb_octopus),
