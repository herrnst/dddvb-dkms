...
static int qam_set_modulation(struct stv_state *state)
{
	...
	case QAM_64:
		...
		if(state->param.u.qam.symbol_rate>4500000)
		{
			writereg(state, R367_QAM_FSM_STATE,0xb0);
			writereg(state, R367_QAM_EQU_CTR_LPF_GAIN,0xc1);
			writereg(state, R367_QAM_EQU_CRL_LPF_GAIN,0xa5);
                }
                else if(state->param.u.qam.symbol_rate>2500000) // 25000000
                {
	...
	case QAM_128:
                ...
                if(state->param.u.qam.symbol_rate>4500000) // 45000000
                {
			writereg(state, R367_QAM_EQU_CRL_LPF_GAIN,0xa7);
                }
                else if(state->param.u.qam.symbol_rate>2500000) // 25000000
...
{
	int status = 0;
	u32 sr = state->param.u.qam.symbol_rate;
	u32 Corr = 0;
	...
	Temp = state->param.u.qam.symbol_rate;
	Temp1 = sr;
...
static int QAM_Start(struct stv_state *state, s32 offsetFreq,s32 IntermediateFrequency)
{
	int status = 0;
        u32 AGCTimeOut = 25;
        u32 TRLTimeOut = 100000000 / state->param.u.qam.symbol_rate;
        u32 CRLSymbols = 0;
        u32 EQLTimeOut = 100;
        u32 SearchRange = state->param.u.qam.symbol_rate / 25;
        u32 CRLTimeOut;
        u8 Temp;
	...
	case QAM_256:  CRLSymbols = 250000; break;
        ...
        CRLTimeOut = (25 * CRLSymbols * (SearchRange/1000)) / (state->param.u.qam.symbol_rate/1000);
        CRLTimeOut = (1000 * CRLTimeOut) / state->param.u.qam.symbol_rate;
